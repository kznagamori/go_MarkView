...
**実装は一通り揃い、`v1.0.0-rc.2` に対して手動テスト 67 件を実施した**（2026-09-05）。起動してファイルを開き、変換して表示し、ツリー・アウトライン・検索・テーマ切り替え・PlantUML の描画・外部エディタで開くまで動く。CI・自動 E2E・リリースワークフローも通っている。

**結果は OK 57 / NG 7 / 対象外 3。** 記録は `docs/tests/results/MarkView_手動テスト結果_v1.0.0-rc.2.xlsx`。

**その後、不具合 7 件をすべて直した**（2026-09-05）。**残っているのは `v1.0.0-rc.3` を打って手動テスト 67 件をやり直すことだけ**である。

> [!IMPORTANT]
> **不具合は 7 件あり、7 件とも修正して実機で確認した**（2026-09-05）。rc.2 の NG 7 件がまとまった **5 件**と、
> **その修正中に見つかった 2 件**（BUG-006 / BUG-007）である。
>
> **後の 2 件は rc.2 にも入っていた。** 手動テスト 67 件を通り抜けており、**「rc.2 で OK だったから無い」とは言えない。**
> **同じ理由で、rc.3 の 67 件も省けない。**
...
| [BUG-005](docs/bugs/2026-09-04-bug-005-about-webview-version.md) | 情報ダイアログに WebView の版が出ない | E2E-301 | `bind.go` と、新設した `webview_windows.go` / `webview_other.go`（IMP-181） | **済**（W1 は実機 OK。**Linux は CI 待ち**） |
...
| 区分 | 内容 |
| --- | --- |
| **ある** | `main.go` とルートの Go ファイル群（Wails との境界）、`internal/` 13 パッケージ、`frontend/`（HTML / CSS / JS / アイコン / **同梱資産は Mermaid・KaTeX・PlantUML の 3 つとも配置済み**）、`go.mod`、`wails.json`、`scripts/`（`copyicons`・`genchroma`・`genlicenses`・`smoke`・`gentestdata`・`e2e`・`pack`・`vendorupdate`・**`domids`**）、`licenses/THIRD_PARTY.md`、`testdata/`（`showcase.md`・`smoke.md`・`e2e/`）、`docs/`（利用者向け 5 文書 + 仕様 20 文書 + `bugs/` の調査報告 8 件）、`assets/`、`.github/workflows/`（`ci.yml`・`release.yml`）、ルート `README.md` |
| **未修正** | **無し。** 7 件とも修正し、実機で確認した（2026-09-05）。仕様・単体テスト仕様・E2E テスト仕様にも反映済み（4.23.0〜4.31.0）|
| **未実施** | **`v1.0.0-rc.3` のリリース CI と、そこでの手動テスト 67 件（全件）。** **41 章を改訂したため rc.2 の結果は引き継げない**（E2E-201）。**Linux 側の確認もここが最初の機会**（BUG-005 の WebKitGTK の版取得は Windows ではコンパイルすら確認できない）|
...
仕様は `specs-4.2.0` タグの時点で一度完成しており、その後の改訂は `docs/specs/README.md` の改訂履歴に記録する（現在 **4.31.0**）。実装時は**仕様を正とし、迷ったら実装ではなく仕様を読む**。仕様と違う判断をしたときは、同じ変更で仕様書を直し、**改訂履歴に 1 行足して版を上げる**（NFR-071）。
...
| `v1.0.0-rc.3` | **未着手。不具合 7 件の修正は完了しており、打てる状態にある**（2026-09-05）。**指示があるまで打たない**（BR-050）|

> [!IMPORTANT]
> **rc.3 では手動テストを全 67 件やり直す**（E2E-201）。41 章を改訂した以上、rc.2 の OK は別のケースに対する結果である。**BR-080 も「rc と本タグの間に検証に関係しない変更を入れない」と定めており、仕様書の改訂はそれに当たる。**
>
