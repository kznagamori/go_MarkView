...
**実装は一通り揃っている。** 起動してファイルを開き、変換して表示し、ツリー・アウトライン・検索・テーマ切り替えまで使える。CI・自動 E2E・リリースワークフローも書き終えた。**残っているのは実地で通すこと**——`v1.0.0-rc.1` を打ってリリース CI を最後まで走らせ、その成果物に対して手動テスト 60 件を実施する（E2E-205）。
...
| **未実施** | **手動テスト 60 件**（41 章、E2E-205）と、それに向けた `v1.0.0-rc.1` のリリース CI。リリース CI 自体は `v0.1.0-rc.1` / `rc.2` で**試験的に動かしてある**（`vendor-update` が KaTeX を 0.18.5 へ上げ、タグを付け替えたコミットがリモートに残っている）。ただし**あれは試験用のタグであり、E2E-205 の検証対象ではない** |
...
仕様は `specs-4.2.0` タグの時点で一度完成しており、その後の改訂は `docs/specs/README.md` の改訂履歴に記録する（現在 **4.5.0**）。実装時は**仕様を正とし、迷ったら実装ではなく仕様を読む**。仕様と違う判断をしたときは、同じ変更で仕様書を直し、**改訂履歴に 1 行足して版を上げる**（NFR-071）。
...
2. その成果物に対して手動テスト 60 件を実施し、結果を `docs/tests/results/` にコミット
