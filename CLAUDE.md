...
**実装は一通り揃っている。** 起動してファイルを開き、変換して表示し、ツリー・アウトライン・検索・テーマ切り替えまで使える。CI・自動 E2E・リリースワークフローも書き終えた。**残っているのは実地で通すこと**——rc タグを打ってリリース CI を最後まで走らせ、その成果物に対して手動テスト 59 件を実施する（E2E-205）。
