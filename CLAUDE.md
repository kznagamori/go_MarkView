...
| [BUG-018](docs/bugs/2026-09-21-bug-018-task-checkbox-accent-color.md) | **L1 でチェック済みのタスクリストのチェックボックスがオレンジになる**（`accent-color` を指定しておらず、色を描画エンジンの既定に委ねていた。WebKitGTK は GTK テーマのアクセント色で描く。**v1.0.0 から**。`v1.1.0-rc.1` の手動テストで利用者が発見） | E2E-231（L1） | `frontend/css/markdown.css` に `accent-color: var(--accent-emphasis)`（DSP-121）。**仕様の不足**（DSP-121 が色を定めていなかった） | **済**（仕様 4.70.0。**修正後の実機での確認は rc.2 の E2E-231**） |
| [BUG-019](docs/bugs/2026-09-21-bug-019-context-menu-copy-webkitgtk.md) | **L1 で右クリックの `Copy` が、選択を解除したうえに空文字をクリップボードへ入れる**（**WebKit は要素へフォーカスを移すと文書の選択範囲を解除する**。メニューを開いた直後に最初の項目へフォーカスを移すため、押す前に選択が消えていた。`v1.1.0-rc.1` の手動テストで利用者が発見し、L1 で切り分けた） | E2E-371（L1） | `frontend/js/contextmenu.js`（開く前に `Range` を控え、フォーカスの直後とコピーの直前に当て直す `restoreRange`。空文字は Go へ渡さない。IMP-249）。**仕様の不足**（IMP-249 が「選択範囲はフォーカスの移動では消えない」を前提にしていた） | **済**（仕様 4.70.0。**修正後の実機での確認は rc.2 の E2E-371**） |
| [BUG-020](docs/bugs/2026-09-21-bug-020-file-drop-webview-navigation.md) | **L1 でドラッグ中の案内が出ず、ドロップすると WebView がそのファイルへ遷移して戻れなくなる**（**WebKitGTK は `dataTransfer.types` に `Files` を入れない**ため `hasFiles()` が偽になり、`preventDefault()` に届かない。Wails の Linux 実装は `onDragDrop` が `FALSE` を返し、既定の処理を止めない。**v1.0.0 から。rc.3 / rc.4 では L1 が WSL でドロップを実施できず、一度も試されていなかった**） | E2E-223 / E2E-381（L1） | `frontend/js/dnd.js`（`dragover` / `drop` を条件なしで `preventDefault()`。`text/uri-list` も受け、`dragstart` / `dragend` でウィンドウ内のドラッグを除く。IMP-245）。**仕様の不足** | **済**（仕様 4.70.0。**修正後の実機での確認は rc.2 の E2E-223 / E2E-381**） |

| 区分 | 内容 |
| --- | --- |
| **ある** | `main.go`（ルート。CLI・埋め込み・Wails の起動）と `desktop/`（Wails との境界。`App` とバインドメソッド）、`internal/` 13 パッケージ、`frontend/`（HTML / CSS / JS / アイコン / **同梱資産は Mermaid・KaTeX・PlantUML の 3 つとも配置済み**）、`go.mod`、`wails.json`、`scripts/`（`copyicons`・`genchroma`・`genlicenses`・`smoke`・`gentestdata`・`e2e`・`pack`・`vendorupdate`・**`domids`**）、`licenses/THIRD_PARTY.md`、`testdata/`（`showcase.md`・`smoke.md`・`e2e/`）、`docs/`（利用者向け 5 文書と索引 + 仕様 20 文書 + `bugs/` の調査報告 21 件 + `tests/`（記録用 Excel の生成スクリプトと実施記録））、`assets/`、`.github/workflows/`（`ci.yml`・`release.yml`）、ルート `README.md` |
| **未修正** | **無し。** BUG-001〜BUG-020 はすべて片づいた（v1.0.0 までの 10 件は 4.23.0〜4.38.0、v1.1.0 の 7 件は 4.43.0〜4.69.0、**`v1.1.0-rc.1` の手動テストで出た 3 件（BUG-018〜020）は 4.70.0**）。**ただし BUG-011〜BUG-020 の修正後の振る舞いは、まだ実機で確かめていない**（BUG-011〜017 は rc.1 で OK だった。**BUG-018〜020 は rc.2 の手動テストで見る**）。**`E2E-332`（Linux のアイコン）は不具合にせず、要求を見直した**——Linux では実行ファイル単体でアイコンを出せないため、**E2E-332 は W1 のみ**になった（UI-025 / DSP-017。4.70.0） |
| **未実施** | **v1.1.0 の手動テスト 86 件を rc.2 の成果物で全件やり直す**（BR-080, E2E-201。41 章を改訂したため、rc.1 の OK は別のケースに対する結果である）。**`v1.1.0-rc.1` では OK 80 / NG 5 / 対象外 1** だった（2026-09-20〜21。**NG 5 件はすべて L1 だけ**で、W1 は全件 OK。**BUG-011〜BUG-017 の修正後の振る舞いは W1 / L1 とも OK**）。v1.0.0 は rc.4 で 67 件を全件実施した（OK 66 / NG 0 / 対象外 1）|
...
仕様は `specs-4.2.0` タグの時点で一度完成しており、その後の改訂は `docs/specs/README.md` の改訂履歴に記録する（現在 **4.70.0**）。実装時は**仕様を正とし、迷ったら実装ではなく仕様を読む**。仕様と違う判断をしたときは、同じ変更で仕様書を直し、**改訂履歴に 1 行足して版を上げる**（NFR-071）。
