...
>
> **`v1.1.0-rc.1` で全行を実機で確かめた**（2026-09-20〜21。手動テスト 86 件。OK 80 / NG 5 / 対象外 1）。**上の前提のうち成り立たなかったのは `document.execCommand('copy')` の行だけである**——**WebKit はフォーカスの移動で文書の選択範囲を解除する**ため、メニューを開いた時点で選択が消え、`Copy` が空になった（L1。[BUG-019](../bugs/2026-09-21-bug-019-context-menu-copy-webkitgtk.md)。4.70.0 で直した）。**BUG-011〜BUG-017 の修正後の振る舞いは、W1 / L1 とも OK だった。**
>
> **rc.1 の手動テストで、ほかに 3 件の不具合と 1 件の要求の見直しが出た**（いずれも **L1 だけ**。W1 は全件 OK）。**修正後は rc.2 の手動テストで見る**:
>
> | 見つかったもの | 直したもの | 確かめ直すケース |
> | --- | --- | --- |
> | **[BUG-018](../bugs/2026-09-21-bug-018-task-checkbox-accent-color.md)**（チェック済みのチェックボックスがオレンジ。`accent-color` を指定していなかった。**v1.0.0 から**） | DSP-121（色を明示する） | **E2E-231 の確認内容 4**（L1） |
> | **[BUG-019](../bugs/2026-09-21-bug-019-context-menu-copy-webkitgtk.md)**（`Copy` で選択が解除され、空文字がクリップボードへ入る） | IMP-249（`Range` を控えて当て直す・空文字を渡さない） | **E2E-371 の確認内容 3・4**（W1 / L1） |
> | **[BUG-020](../bugs/2026-09-21-bug-020-file-drop-webview-navigation.md)**（案内が出ず、ドロップで WebView が遷移して戻れない。**v1.0.0 から。L1 でドロップを一度も試していなかった**） | IMP-245（条件なしで `preventDefault`・`text/uri-list`・内部ドラッグの旗） | **E2E-223 の確認内容 1〜4** と **E2E-381 の確認内容 10**（L1。W1 も退行を見る） |
> | **E2E-332**（L1 でアイコンが歯車）。**不具合にしない**——Linux では実行ファイル単体でアイコンを出せない | UI-025 / DSP-017 / E2E-332（Linux を対象外とし、`troubleshooting.md` に `.desktop` の例を載せる） | **E2E-332 は W1 のみ**になった |
