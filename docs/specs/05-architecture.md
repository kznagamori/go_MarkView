...
>
> したがって Linux では WebKitGTK の既定（`$XDG_DATA_HOME` と `$XDG_CACHE_HOME` の配下）に作られる。これは NFR-033 が禁じている領域に当たるため、**NFR-033 の明示的な例外として扱う**。実際の位置は E2E-313 で実測し、判明した内容をこの表へ反映する。
>
