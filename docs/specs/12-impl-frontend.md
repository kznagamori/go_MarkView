...
- SVG は `index.html` の先頭に `<svg style="display:none">` のシンボル定義としてまとめ、各ボタンは `<svg><use href="#icon-open"></use></svg>` で参照する。
- `fill="currentColor"` とし、色は CSS から与える（DSP-014）。
- アイコンの一覧と対応は以下とする。同じ絵柄を複数箇所で使う場合、シンボルは 1 つだけ定義して共用する。
...
async function boot() {
  ...
  initToolbar(); initPanes(); initShortcuts(); initDnd(); initSearch();
...
- インデントは `level` に応じた CSS カスタムプロパティで与える（DSP-113）。
...
```js
// js/zoom.js
export function setZoom(percent)  // 50..300、10 刻みに丸める
```
...
- `Ctrl` + ホイールは `wheel` イベントで `ctrlKey` を見て処理し、`preventDefault()` でブラウザ既定のズームを抑止する。
- 変更後、Go 側へ通知して保存させる（IMP-310）。
...
- `#app` の `data-theme` 属性を書き換えるだけで全体に反映する。CSS 変数の切り替えで完結させ、要素の再生成や本文の再変換を行わない（UI-105）。
- Mermaid のみ再描画が必要（IMP-231）。
- 変更後、Go 側へ通知して保存させる。
...
- 入力欄（検索バー）にフォーカスがある間は、テキスト編集に関わるキーを素通しする（UI-090）。
- 各ハンドラは `preventDefault()` を明示的に呼ぶ。
...
// state.outlineSuppressed … 幅不足による一時的な非表示（保存しない）
// 実際の表示 = outlineVisible && !outlineSuppressed
```

- `resize` イベントで本文ペインの幅を算出し、240 px を下回るなら `outlineSuppressed = true`、回復したら `false` に戻す。
- **`outlineVisible` と設定値、ツールバーの `aria-pressed` は変更しない。** ここを変えてしまうと、ウィンドウを広げても戻らなくなり、設定にも誤った値が保存される。
...
- `confirm-large` の `Open anyway` ボタンは、押下で `api.openFile(path, { confirmed: true })` を呼ぶ（IMP-314）。
...
- 表示中は背後をフォーカストラップし、`Tab` がダイアログ外へ出ないようにする。
- `Esc` またはオーバーレイ外クリックで閉じ、フォーカスを `#btn-about` へ戻す。
