...
仕様書は 3 つの層に分かれる。上位の層が下位の層を規定し、下位が上位と矛盾する場合は**上位を正とする**。
...
flowchart TD
    ...
    TRACE["90 トレーサビリティ<br/>3 層の ID 対応表"]
...
| # | 文書 | 内容 |
| --- | --- | --- |
| 90 | [トレーサビリティ](90-traceability.md) | 要求 ↔ 実装 ↔ 表示 の ID 対応表（正引き・逆引き・カバレッジ） |
...
| 仕様を変更する | 要求仕様を直す → [90](90-traceability.md) で影響する `IMP`/`DSP` を特定 → 該当箇所を直す |
