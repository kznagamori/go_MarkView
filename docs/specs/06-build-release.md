...
- **資産の決め打ち id と `index.html` の id が、文書の id の接頭辞（`user-content-`。AR-053）で始まらないことも確かめる。** 始まっていると、文書の見出しや生 HTML の `id` と衝突しうる（[BUG-011](../bugs/2026-09-14-bug-011-document-id-collision.md)）。
...
| 2 | 資産が決め打ちする id と `index.html` の id の**どれも `user-content-` で始まらない**（AR-053） | **リリースを中止する。** 文書の id と衝突しうる。**文書は実行時に来るため、衝突そのものはこの検査では見えない**——名前空間が分かれていることだけを確かめる（BUG-011） |
...
6. 成功した場合、1〜4 の変更をコミットする。**既定ブランチ（`main`）の先端がタグの指していたコミットと一致するときだけ**、そのブランチへ push する（BR-050 の表の「ブランチへの push」）。一致しない場合は警告を出し、**資産の更新はタグの側にだけ残る**——開発者は E2E-205 の手順に従ってそのコミットを取り込む。
...
> - タグの付け替えは**リリースが公開される前**に行われるため、利用者が取得済みのタグを書き換えることはない。ただし、タグを push した開発者のローカルリポジトリとは差異が生じるため、**rc のタグを push した後は `git fetch --tags --force` でタグが動いたかを確かめ、動いていればそのコミットを作業ブランチへ取り込む**（手順は E2E-205）。**rc のタグは `main` の先端に打つ**——先端でなければ上の手順 6 で push されず、資産の更新がどのブランチにも載らない（v1.0.0-rc.4 で実際に起きた）。
...
```mermaid
flowchart TD
    T["git push --tags"] --> V["vendor-update<br/>ubuntu-latest<br/>（どちらのタグでも通る）"]
    V --> P{"タグの種類"}
    P -->|"v1.0.0-rc.1<br/>プレリリース"| F{"Mermaid / KaTeX / PlantUML<br/>最新版の取得"}
    P -->|"v1.0.0<br/>本リリース"| SKIP["資産の更新をスキップ<br/>(BR-043)"]
    F -->|"取得に失敗 / 既に最新"| K["コミット済みの版で継続<br/>リリースノートに記録"]
    F -->|"許容しないライセンス / 告知の食い違い<br/>(license-rejected / notice-mismatch)"| X["リリース中止<br/>リポジトリは変更しない"]
    F -->|成功| D["決め打ち id の検査<br/>(scripts/domids)"]
    D -->|失敗| X
    D -->|成功| S["描画スモークテスト<br/>(BR-054)"]
    S -->|失敗| X
    S -->|成功| C["vendor / vendor.json /<br/>ライセンスをコミット<br/>（main の先端がタグと同じなら push）<br/>タグを新コミットへ付け替え"]
...
| ジョブ | ランナー | 役割 |
| --- | --- | --- |
| `vendor-update` | `ubuntu-latest` | 埋め込み資産の更新、告知の照合、決め打ち id の検査、スモークテスト、コミットとタグの付け替え（BR-043, BR-054）。**本タグでも走る**（ビルドするコミットを確定させる。下表） |
...
- 本リリースで rc を基準にしてはならない。E2E-205 と BR-080 が「rc と本タグの間に別の変更を入れない」（手動テストの記録を除く）と定めているため、**変更点がほぼ空になる**。利用者は rc のノートを遡らないと何が入ったか分からなくなる。
...
- **本文の id の検査は、PlantUML の描画がすべて終わってから行う**こと。`plantuml.js` はログを出すたびに `#status` を書き換えるため、描画の前に見ると衝突していても通る
- **並べ替えの比較の期待値は、`scripts/smoke` に人が書いたリテラルで持つ**こと（UT-031 と同じ理由。実装を写した式で作らない）
- **目印の照合は「対象にしないこと」を先に見る**こと。「すべてを対象にしない」実装でも GFM の要素の側の検査で落ちるよう、**両方向を必ず見る**（UT-209 の IMPORTANT と同じ）。図のブロックでも同じく、**偽装したブロックが描画されず、本物のブロックは描画される**ことの両方を見る
- **本文の id の検査は、要素の名前空間（XHTML / SVG）をページから受け取って Go 側で判定する**こと。ページの側で SVG の中を除外すると、Mermaid のラベルに書いた `id`（XHTML 名前空間）まで検査から外れる
...
- rc と本タグの間に、検証に関係しない変更（文書の修正など）も入れない。入れた場合は rc をやり直す。**例外は、その rc の手動テストの記録（`docs/tests/results/` の Excel 1 ファイル）だけである**（E2E-205 の手順 6）。記録は検証の結果であって、検証の対象を変えない。
- **本タグを打つ前に、rc のタグとの差がその記録だけであることを確かめる**（`git diff --name-only <rc タグ> HEAD`。E2E-205）。rc のタグは CI が資産の更新コミットへ付け替えることがある（BR-043）ため、**付け替えられたコミットを取り込まずに本タグを打つと、検証していない資産が配布される**（v1.0.0-rc.4 で、取り込む前の作業ブランチには KaTeX 0.18.5 が残っていた）。
