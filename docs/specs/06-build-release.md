...
CI において、生成スクリプトを実行した結果と、リポジトリにコミットされている `licenses/THIRD_PARTY.md` を比較し、**差分がある場合はビルドを失敗させる**。依存を追加・更新したときのライセンス表示の更新漏れを防ぐため。
...
| ライセンス鮮度（BR-041） | 生成結果との差分がないこと | 失敗 |
| バイナリサイズ（BR-060） | 計測して記録すること | 警告のみ（CI は失敗させない） |
| 描画スモークテスト（BR-054） | `frontend/vendor/` に変更を含む PR で実行 | 失敗 |
