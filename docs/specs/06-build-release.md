...
Mermaid と KaTeX は、**実ファイルをリポジトリにコミットして管理する**。

| 項目 | 規定 |
| --- | --- |
| 配置場所 | `frontend/vendor/` |
| バージョンの記録 | `frontend/vendor/vendor.json` に、資産ごとの名称・バージョン・取得元 URL・取得日を記録する |
| 取得元 | 公式配布物（npm レジストリの tarball、または公式 CDN の該当バージョン）に限る |
| 改変 | 一切行わない。取得したファイルをそのまま格納する |
...
2. `vendor.json` を更新する。
3. ライセンス生成（BR-040）を実行する。
4. 描画スモークテスト（BR-054）をローカルで実行し、表示を確認する。
5. 以上を 1 つのコミットとしてリポジトリに入れる。

### BR-043: リリース時の埋め込み資産の自動更新 **MUST**

**プレリリースタグ（`v<version>-rc.<n>`）の付与を契機に**、CI が Mermaid / KaTeX をその時点の最新安定版へ自動更新する。これにより、開発者が明示的に更新しなくても、リリースのたびに新しい Mermaid が取り込まれる。
...
**取得の範囲**: メジャーバージョンの更新を含めて、常に最新安定版を取得する（Mermaid / KaTeX 側のプレリリース版は対象外）。

**取得に成功した場合**の処理順序は以下とする。

1. 最新安定版を取得し、`frontend/vendor/` を更新する。
2. `vendor.json` を更新する。
3. ライセンス情報（BR-040）を再生成する。
4. **描画スモークテスト（BR-054）を実行する。**
   - 失敗した場合、**リリースを中止する**。この時点ではまだコミットも push も行っていないため、リポジトリの状態は変化しない。CI を失敗として終了する。
5. 成功した場合、1〜3 の変更をコミットし、タグが指していたコミットを含むブランチ（既定は `main`）へ push する。
6. **タグを新しいコミットへ付け替え**、強制更新で push する。
7. 付け替え後のコミットからビルドし、リリースを作成する。
...
> [!NOTE]
> 「常に最新を取り込む」設計は、Mermaid の新しい図種別や不具合修正を自動的に取り込める一方、メジャーバージョンアップによる破壊的変更をリリース直前に抱え込むリスクを伴う。このリスクは BR-054 のスモークテストで受け止める。スモークテストが失敗した場合は、開発者が `vendor.json` のバージョンを明示的に固定したうえでタグを打ち直す運用とする。

手順 1〜2（取得と `vendor.json` の更新）は `scripts/vendorupdate` が行う。手順 3 以降はワークフローの担当とする。
...
| 取り出すもの | Mermaid は `dist/mermaid.min.js` と `LICENSE`。KaTeX は `dist/katex.min.js` / `dist/katex.min.css` / `dist/fonts/` 一式と `LICENSE` |
| 欠落の扱い | 要るものが 1 つでも tarball に無ければ**失敗**とする。版が上がって配置が変わったことに気づかないまま、欠けた資産で配布するのが最悪の結末である |
| 片方だけの更新 | **しない。** 2 つのうち 1 つでも取れなければどちらも更新しない。半端に新しい組み合わせを作らず、リリースノートに書く内容も 2 通りで済む |
| 置き換え方 | 一時ディレクトリへ取得し、**全部そろってから**置き換える。置く前に既存を消す（版によってフォントの構成が変わりうるため） |
| 失敗時の終了コード | **0。** 取得の失敗でリリースを中止しないため、呼び出し側が判断できるよう `updated` と `reason`（`updated` / `already-latest` / `fetch-failed`）を返す |
...
| ブランチへの push | **既定ブランチの先端がタグのコミットと一致するときだけ行う。** 先へ進んでいる場合に無理やり載せると、リリースと関係のない変更を巻き込むか、非早送りで弾かれる。一致しない場合は警告を出し、資産の更新はタグの側にだけ残す |
...
| 変更点の一覧（BR-051） | 直前のタグからの `git log` で作る。`--generate-notes` に頼らず、本文を全部こちらで組み立てる |

### BR-051: リリースノート **MUST**

Release の本文には少なくとも以下を含める。

- 変更点の一覧（コミットまたは PR から自動生成してよい）
- 各成果物の対応環境
...
- SHA-256 チェックサムの確認方法
- **同梱した Mermaid / KaTeX のバージョン**。自動更新（BR-043）が行われた場合は更新前後のバージョンを、取得に失敗した場合はその旨と使用したバージョンを記載する
