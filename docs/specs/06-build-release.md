...
タグの付与を契機に、CI が Mermaid / KaTeX を**その時点の最新安定版へ自動更新**する。これにより、開発者が明示的に更新しなくても、リリースのたびに新しい Mermaid が取り込まれる。

**取得の範囲**: メジャーバージョンの更新を含めて、常に最新安定版を取得する（プレリリース版は対象外）。
...
```mermaid
flowchart TD
    T["git tag v1.1.0<br/>git push --tags"] --> V["vendor-update<br/>ubuntu-latest"]
    V --> F{"Mermaid / KaTeX<br/>最新版の取得"}
    F -->|失敗| K["コミット済みの版で継続<br/>リリースノートに記録<br/>(BR-043)"]
    ...
    K --> B2
    K --> B3
    B1 --> R["release<br/>チェックサム生成<br/>GitHub Release 作成"]
    B2 --> R
    B3 --> R
```
