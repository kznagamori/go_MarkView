...
フロントエンドで動くのは Mermaid・PlantUML・KaTeX だけです。
...
HTML の `id` 属性には、このスラッグの前に **`user-content-`** が付きます
（`## What's New?` なら `id="user-content-whats-new"`）。GitHub と同じ出力です。
リンクは `#whats-new` と `#user-content-whats-new` のどちらで書いても、同じ見出しへ移動します。

> [!NOTE]
> 接頭辞を付けるのは、文書の中の `id` が MarkView の画面の部品と同じ名前になっても、
> 画面の表示や操作を乗っ取らないようにするためです。スラッグそのもの（上の規則）は変わりません。

## リストとタスクリスト

- 順序なし・順序付き・入れ子のいずれにも対応します。
- タスクリスト `- [ ]` / `- [x]` はチェックボックスとして描画します。
  **編集モードでない間は読み取り専用**で、クリックしても状態は変わりません。
  編集モードの間だけ、クリックで切り替えてファイルへ書き込みます
  （[使い方](./usage.md#チェックボックスを切り替える)）。
...
- 見出し行を除いて 2 行以上ある表は、見出しの右端のボタンで**表示上だけ**並べ替えられます
  （[使い方](./usage.md#表を並べ替える)）。ファイルは書き換えません。
- 編集モードの間は、セルの文字を書き換えられます（[使い方](./usage.md#表のセルを直す)）。
- 生 HTML で書いた表（`<table>`）は、並べ替えも書き換えもできません。
...
- 図は本文幅に収まるよう縮小し、縦横比を保ちます。右上のボタンで、原寸表示と拡大画面に
  切り替えられます（[使い方](./usage.md#図と画像を大きく見る)）。
...
> クリックのコールバックが実行されることはありません。**
>
> `click` で URL を指定したノードと、ノードのラベルに書いたリンクは、**本文のリンクと同じ扱い**です
> （`http://` / `https://` は既定のブラウザで開きます。[使い方](./usage.md#リンクをたどる)）。
> ウィンドウの中で別のページへ移ることはありません。拡大画面の中では、押しても何も起きません。

## PlantUML 図

言語指定が `plantuml` または `puml` のコードブロックを図として描画します。
...
- **`uml` は対象になりません。** `plantuml` か `puml` と書いてください（大文字小文字は問いません）。
- 図は本文幅に収まるよう縮小し、縦横比を保ちます。テーマにも追従します。
- 右上のボタンで、原寸表示と拡大画面に切り替えられます。**エラー図にも出ます。**
  下の「描けないもの」で図にならなかったブロックには出ません。
- コピーボタンを押すと、描画された図ではなく**元の PlantUML ソース**がコピーされます。
- 図を含む文書を開いたときにだけ PlantUML を読み込みます。同梱している処理系は
  **5 MB を超える**ため、含まない文書では読み込みません。

**構文に誤りがあるときは、PlantUML が描いたエラー図がそのまま出ます。** 行番号と該当行が
図の中に書かれています。Mermaid が文字でエラーを出すのとは見え方が違いますが、これは
処理系の返し方の違いです。

### 描けないもの

同梱しているのは PlantUML 本体（Java 版）ではなく、**JavaScript へ移植された版**です。
次のものは描けません。

| 描けないもの | どうなるか |
| --- | --- |
| `!include` `!includeurl` `!includesub` `!import` と、`from` を伴う `!theme` | 図にならず、**理由と元のソースが出ます。外部への接続は行いません** |
| 標準ライブラリ（C4 モデル・awslib・tupadr3 など） | `!include <C4/...>` は取り込み指令なので、上と同じ扱いです |
| `salt`（UI モック）・`ditaa` | **エラー図が出るか、理由と元のソースが出るか**のどちらかです |
| 幅か高さが 4096 px を超える図 | 同上。**Java 版には無い制限**です |

> [!NOTE]
> **`from` を伴わない `!theme plain` は使えます。** 組み込みのテーマは外部を取りに行かないためです。

描けなかったブロックは、**元のソースがコードブロックのまま残り、上に理由が出ます。**
他の図や本文の描画は止まりません。
...
- `http://` `https://` のリモート画像も表示します（このときだけ WebView が外部へ接続します）。
  取得した画像は WebView のキャッシュに残ることがあります
  （[設定と保存されるもの](./settings.md#webview-のキャッシュに残るもの)）。
- 画像が見つからない・読めない場合は、代替テキスト（alt）とプレースホルダを表示します。
  **文書全体の描画は止まりません。**
- 本文幅を超える画像は、縦横比を保って縮小します。縮小した画像には、右上に原寸表示と拡大画面の
  ボタンが出ます（[使い方](./usage.md#図と画像を大きく見る)）。画像そのもののクリックでは何も起きません。
...
例外はチェックボックスで、`type="checkbox"` と `checked` / `disabled` に限って通します。
スクリプトは伴いません。**生 HTML で書いたチェックボックスは、編集モードの間も切り替えられません。**
切り替えられるのは、タスクリスト（`- [ ]`）のチェックボックスだけです。

**生 HTML で、タスクリストや表、図のブロックに見せかけた要素を書いても、MarkView はそれを
書き換えの対象にも、図としての描画やコピーの対象にもしません。** どれが本物かを、変換のたびに作る
鍵で見分けているためです（文書の書き手は、その鍵を前もって知りようがありません）。

`id` 属性は、見出しと同じく前に `user-content-` を付けて出力します（`h1`〜`h6`・`li`・`sup`・`div` に限る）。
`<div id="note">` へは、`#note` のリンクで移動できます。
